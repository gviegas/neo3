...
func (cs *Commands) Filter() {
	...
	// As of v1.2, all core commands end with either a lowercase letter or a
...
func (cs *Commands) GenCCode() (hdr, src string) {
	...
	switch runtime.GOOS {
	default:
		def = "// XXX: No platform-specific definitions for " + runtime.GOOS
	case "android":
		def = "#define VK_USE_PLATFORM_ANDROID_KHR"
	case "linux":
		def = "#define VK_USE_PLATFORM_WAYLAND_KHR\n#define VK_USE_PLATFORM_XCB_KHR"
	case "windows":
		def = "#define VK_USE_PLATFORM_WIN32_KHR"
	}
	hdr = fmt.Sprintf(CHeader, def, cs.GenFPs(true), cs.GenCGetProcs(true), cs.GenCWrappers())
	src = fmt.Sprintf(CSource, cs.GenFPs(false), cs.GenCGetProcs(false))
	return
}
